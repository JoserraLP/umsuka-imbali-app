...
| `20260101006400_member_payments.sql` | ENUM `umsuka.payment_type` (`monthly`/`yearly`) + tabla `umsuka.member_payments` (`user_id` FK SET NULL, `payment_type`, `period_month` 1-12 nullable, `period_year` 1-9999, `amount numeric(10,2) >0`, `paid_at date`, `registered_by`, `notes` <=2000, `created_at`, índices únicos parciales monthly/yearly para deduplicación, RLS híbrida `is_management() OR user_id=auth.uid()` en SELECT y `is_management()` en ALL) + `ALTER TYPE event_type ADD VALUE 'material_distribution'` — Sprint 31 |
| `20260101006500_rehearsal_auto_enroll.sql` | ENUM `umsuka.rehearsal_category` (`music`/`dance`) + `events.rehearsal_category` nullable + CHECK `event_type='rehearsal' OR rehearsal_category IS NULL` + `rehearsal_attendance.enrolled` bool + `enrolled_at` timestamptz + CHECK coherencia + índice `event_id,enrolled` + RLS sin cambios (write `is_management()`) — Sprint 32 |
| `20260101006600_rehearsal_attendance_service_role.sql` | Grants `service_role` SELECT/INSERT/UPDATE/DELETE en `umsuka.rehearsal_attendance` + SELECT en `umsuka.events` para auto-enroll — Sprint 32 follow-up |
| `20260101006700_dance_formation_instruments.sql` | Tablas `umsuka.dance_formations` (name 1-200, event_id SET NULL), `umsuka.dance_positions` (row>=1, seat 1-6, UNIQUE seat + UNIQUE miembro parcial, FKs SET NULL/CASCADE), `umsuka.musician_instruments` (user_id, instrument_id, formation_id NULL, UNIQUE user+formation parcial + UNIQUE user global + UNIQUE instrument+formation), índices, RLS ENABLE+FORCE (SELECT authenticated true, ALL is_management()), comentarios, grants — Sprint 33 |
| `20260101006800_formation_type.sql` | ENUM `umsuka.formation_type` (`dance`/`music`) + `dance_formations.formation_type` NOT NULL default dance + índices + helper `current_user_component_type()` + RLS SELECT por componente (`formation_type = current_user_component_type() OR is_management()`) para `dance_formations`, `dance_positions` (via EXISTS JOIN) y `musician_instruments` (join o global music) — sin pasillo, filas dinámicas, separación baile/música — Sprint 33 fix |
